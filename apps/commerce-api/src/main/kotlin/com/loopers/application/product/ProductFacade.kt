...
class ProductFacade(
    ...
    fun getDetail(userId: UserId, productId: Long): Result.ProductResult {
        val cachedInfo = productCacheTemplate.get(ProductKey.GetProduct(productId))
        if (cachedInfo != null) {
        }
        ...
        val likes = likeRepository.findLikedProductIds(userId, listOf(productWithSignal.product.id))
        val info = productCacheTemplate.save(factory.generateInfo(productWithSignal, brand))
